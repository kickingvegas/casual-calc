;;; casual-fileio.el --- Casual File I/O Routines    -*- lexical-binding: t; -*-
...
;;(kill-new (pp (casual-datafile-to-matrix "~/fred.dat")))

(defun casual-matrix-to-calc-vector (matrix)
  "Convert matrix into a Calc vector MATRIX.

This function converts the output of `casual-datafile-to-matrix'
to a Calc matrix."
  (let ((templist (list)))
    (mapc (lambda (x)
...
(defun casual-read-curvefit-data (filename)
...
- `casual-calc-curve-fit'"
  ...
  (calc-transpose
   (calc-push
    (casual-matrix-to-calc-vector
     (casual-datafile-to-matrix filename)))))

(defun casual-read-plot-data (filename)
...
data can be plotted using either `casual--graph-add' or
`casual--graph-add-3d'."
...
(provide 'casual-fileio)
;;; casual-fileio.el ends here
